...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // Since the node is a vector of unique pointers,
                        // the parameter in the lamba is passed by reference.
    ...
    ChatBot robo("../images/chatbot.png");

    // pass the local copy by reference to the _chatBot pointer for GUI.
    _chatBot = &robo;

    // Set the chatlogic handle of the chatbot
    robo.SetChatLogicHandle(this);

    // add the chatbot to the rootnode of the graph
    robo.SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(robo));
